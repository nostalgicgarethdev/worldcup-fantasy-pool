  ...
  const content = (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white">Demo result entry (local only). Changes affect leaderboard + profiles immediately in this browser.</p>
      </div>
      ...
      <div className="glass p-6">
        <div className="flex items-center gap-2 mb-4 text-sm">
          ...
          {(['group','r32','all'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1 rounded-full border ${filter===f ? 'bg-white/10 border-white/30 text-white' : 'border-white/10 text-white hover:text-white'}`}>{f}</button>
          ))}
          <span className="ml-auto text-xs text-white">Results saved to localStorage • refresh to propagate everywhere</span>
        </div>
            ...
            return (
              <div key={m.id} className="glass flex flex-col sm:flex-row sm:items-center gap-3 p-4 text-sm">
                <div className="flex-1">
                  ...
                  <span className="ml-2 text-xs text-white">#{m.id} • {m.date}</span>
                </div>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setResult(m.id, null)} className="px-3 py-1.5 rounded-xl border border-white/15 text-white hover:text-white">Clear</button>
                </div>
                {cur && <div className="text-xs uppercase tracking-widest text-white self-center">set: {cur}</div>}
              </div>
            )
          })}
        </div>
        <div className="mt-4 text-xs text-white">Tip: After setting results, open Leaderboard or Profile in another tab (or refresh) to see updated scores. Real admin will be authenticated + persist server-side.</div>
      </div>

      <div className="glass p-6 text-sm text-white">
        Treasury: <span className="font-mono text-white">{config.treasuryWallet}</span><br />
    ...
    return (
      <div>
        <div className="glass max-w-md p-7 text-sm mb-4">
          ...
          <span className="text-white">Showing demo controls anyway for local testing.</span>
        </div>
        {content}
      </div>
    )
